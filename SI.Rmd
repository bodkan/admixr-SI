...
present-day European individuals using the following $f_4$-ratio statistic:
...
omit these steps in this Appendix).
...
parsing work (steps 1-4 above) automatically, returning a simple R dataframe:
...
We want to keep only those sites that fall outside of those regions and saving
them in a `snp` format again.
...
snps <- eigenstrat("data/snps") # trio of EIGENSTRAT files is: snps.{ind,snp,geno}
...
eigenstrat("data/snps") %>% # trio of EIGENSTRAT files is: snps.{ind,snp,geno}
